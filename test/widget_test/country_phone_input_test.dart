import 'package:flutter/cupertino.dart'
    show CupertinoButton, CupertinoDynamicColor, CupertinoColors;
          ...
          (tester) async {
            ...
            await tester.pumpWidget(
              createWidgetUnderTest(
                locale: const Locale('en'),
                builder: (_) =>
                    Scaffold(body: CountryPhoneInput(initialCountry: ru)),
              ),
            );
            await tester.pumpAndSettle();

            // Enter some digits to ensure they are cleared after selection.
            final fieldFinder = find.byKey(phoneFieldKey);
            expect(fieldFinder, findsOneWidget);
            await tester.enterText(fieldFinder, '9123456789');
            await tester.pump();
            expect(
              find.textContaining('912'),
              findsWidgets,
            ...
            // Tap Armenia tile (key from e164_key constant '374-AM-0')
            const armeniaKey = ValueKey<String>('374-AM-0');
            expect(find.byKey(armeniaKey), findsOneWidget);
            await tester.tap(find.byKey(armeniaKey));
            await tester.pumpAndSettle();
            ...
            // Phone code updated
            expect(find.textContaining('+374'), findsOneWidget);

            // Text field cleared
            final fieldWidget = tester.widget<TextFormField>(fieldFinder);
            final currentText = fieldWidget.controller?.text;
            expect(currentText, isEmpty);
          },
        );

        testWidgets(
          'should show snackbar and keep previous selection if tapped country has no mask',
          (tester) async {
            final initialCountryRU = getCountryByISO2asJSON('RU');
            ...
            // Tap Vanuatu (no mask) key '678-VU-0'
            const vanuatuKey = ValueKey<String>('678-VU-0');
            expect(find.byKey(vanuatuKey), findsOneWidget);
            await tester.tap(find.byKey(vanuatuKey));
            await tester.pumpAndSettle();
            ...
            final context = tester.firstElement(find.byType(CupertinoButton));
            final snackbarWidget = tester
            ...
            // Selection should remain RU (phone code +7 still visible)
            expect(find.textContaining('+7'), findsOneWidget);
            expect(find.textContaining('+678'), findsNothing);
          },
        );
      });
    });
